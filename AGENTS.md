...
## 项目定位
- 本项目是 C# WinForms / .NET 10 的 Terraria 分段计时器，主工程在 `TerrariaSplit/`，测试工程在 `test/`，发布输出默认写到 `publish/`。
- 开发时先理解现有信息流和模块边界，再改代码；不要为了局部任务引入另一套并行架构。
- 可以参考 Terraria 源码 `..\reference\Terraria1456`，但只把必要事实沉淀成项目内模型、局部复刻或小注释。
- 金字塔种子预筛已经整合在主工程 `TerrariaSplit/Terraria/WorldGeneration/` 内；外层独立 `WorldGenSim/` 历史项目已移除，不要重新引入第二套模拟入口。
...
- 发布主程序：`dotnet publish TerrariaSplit\TerrariaSplit.csproj -c Release -o publish`
...
- `TerrariaSplit/Models/`：跨层共享的简单记录、定义和 run/boss 数据模型。
- `TerrariaSplit/UI/`：WinForms shell、输入适配、窗口/overlay 生命周期和 shell 副作用。
- `TerrariaSplit/UI/Settings/`：设置窗口的分页面 UI、控件工厂、页面生命周期和设置编辑适配。
- `TerrariaSplit/UI/Rendering/`：overlay 绘制模型、布局和渲染器。
- `TerrariaSplit/Terraria/`：Terraria 进程、内存、窗口、存档和自动化集成。
- `TerrariaSplit/Terraria/Process/`：Terraria 客户端/服务器进程定位。
- `TerrariaSplit/Terraria/WorldGeneration/`：金字塔种子预筛、世界生成局部复刻、金字塔/箱子结果模型。
- `TerrariaSplit/Terraria/Automation/`：自动建人物/建世界/进世界流程和筛塔自动化编排。
- `TerrariaSplit/Terraria/Memory/`：Terraria 进程内存读取、签名扫描、世界生成状态和创图种子读取。
- `TerrariaSplit/Configuration/`：设置模型、默认值、序列化、profile 和归一化。
- `TerrariaSplit/Storage/`：运行统计和 split time 持久化。
- `TerrariaSplit/Statistics/`：统计表格和展示行构造，不负责持久化。
- `TerrariaSplit/Localization/`：中英文 UI 字符串、语言名和 Terraria 语言代码映射。
- `TerrariaSplit/Infrastructure/`：JSON 文件 helper、日志、性能诊断、高精度调度和 layered window 平台封装。
- `TerrariaSplit/Assets/`：随程序复制的 boss 图标、参考时间和 Terraria 世界名资源。
