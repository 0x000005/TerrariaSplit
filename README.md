...
TerrariaSplit 是一个为《Terraria》速通练习和计时设计的 Windows 分段计时器。它通过读取游戏进程状态自动识别世界、菜单和 BOSS 击败状态，并在主界面显示分段、参考时间、差值、BOSS 图标和完成动画。

项目目前面向个人使用和本地配置优先的工作流：配置文件、参考时间、上一局记录都以 JSON 文件保存在程序目录中，方便手动备份、编辑和切换。
