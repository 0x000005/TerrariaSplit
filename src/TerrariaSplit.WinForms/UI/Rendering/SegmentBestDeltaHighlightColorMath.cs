...
internal static class SegmentBestDeltaHighlightColorMath
{
    public static Color Apply(Color baseColor, string id, double seconds)
    {
        string style = SegmentBestDeltaHighlightStyles.Normalize(id);
        if (style == SegmentBestDeltaHighlightStyles.None)
        {
            return baseColor;
        }
        ...
        return style switch
        {
            SegmentBestDeltaHighlightStyles.Rainbow =>
                Blend(baseColor, FromHsv((float)(seconds * 150.0), 0.78f, 1f), 0.36f),
            SegmentBestDeltaHighlightStyles.Aurora =>
                Blend(baseColor, FromHsv(229f + MathF.Sin((float)(seconds * 1.45)) * 63f, 0.58f, 1f), 0.34f),
            _ => baseColor
        };
    }
