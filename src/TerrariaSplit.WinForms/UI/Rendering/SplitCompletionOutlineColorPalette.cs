...
internal static class SplitCompletionOutlineColorPalette
{
    public static Color[] GetColors(string id, double seconds)
    {
        string style = SplitCompletionOutlineStyles.Normalize(id);
        float shift = (float)(seconds * GetSpeed(style));
        return style switch
        {
            SplitCompletionOutlineStyles.Rainbow => Hues(shift, 0.84f, 1f, 0f, 95f, 190f, 285f),
            SplitCompletionOutlineStyles.Aurora => AuroraColors(shift),
            SplitCompletionOutlineStyles.Gold => GoldColors(shift),
            _ => [Color.Transparent, Color.Transparent, Color.Transparent, Color.Transparent]
        };
    }
    ...
    private static double GetSpeed(string style)
    {
        return style switch
        {
            SplitCompletionOutlineStyles.Rainbow => 150.0,
            SplitCompletionOutlineStyles.Aurora => 142.5,
            SplitCompletionOutlineStyles.Gold => 165.0,
            _ => 0.0
        };
    }
