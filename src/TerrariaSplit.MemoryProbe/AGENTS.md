...
- 命令行接口当前是 `runtime-layout <pid>`；修改参数、退出码或 JSON 字段时，同步更新调用方和测试。
...
- 需要真实 Terraria 进程时，手动运行：`dotnet run --project src\TerrariaSplit.MemoryProbe\TerrariaSplit.MemoryProbe.csproj -- runtime-layout <pid>`
