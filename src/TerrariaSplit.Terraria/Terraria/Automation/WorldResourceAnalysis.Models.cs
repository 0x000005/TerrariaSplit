...
internal sealed record LocationRecord(
    ...
    string Note,
    int Units = 1,
    double WeightedCost = double.PositiveInfinity,
    int CostLimit = 400,
    bool Qualifies = false,
    int TravelSteps = 0,
    int DugTiles = 0,
    int? NearestRouteX = null,
    int? NearestRouteY = null,
    IReadOnlyList<Point>? AccessPath = null,
    int? ClusterMinX = null,
    int? ClusterMinY = null,
    int? ClusterMaxX = null,
    int? ClusterMaxY = null)
{
    public static LocationRecord Tile(string category, string label, int x, int y, ushort tileId, bool inRoute, string note)
    {
        return new LocationRecord(category, label, "Tile", x, y, tileId, null, null, null, null, null, null, inRoute, note);
    }

    public static LocationRecord Chest(
        string category,
        string label,
        int x,
        int y,
        ...
        string note)
    {
        return new LocationRecord(category, label, "Chest", x, y, null, itemId, chestX, chestY, chestStyle, slot, stack, inRoute, note);
    }

    public static LocationRecord ChestMarker(
        string category,
        string label,
        int x,
        int y,
        ...
        string note)
    {
        return new LocationRecord(category, label, "Chest", x, y, null, null, chestX, chestY, chestStyle, null, null, inRoute, note);
    }
}
...
internal sealed record RouteResult(
    ...
    ushort[]? PassableMasks = null)
{
    public bool IsNearRoute(int tileX, int tileY, int radius)
    {
        if (RouteCellCount == 0)
        {
            return false;
        ...
        int cx = tileX / CellSize;
        int cy = tileY / CellSize;
        int cellRadius = Math.Max(1, (int)Math.Ceiling(radius / (double)CellSize) + 1);
        double maxDistance = radius + CellSize * 0.75d;
        double maxDistanceSq = maxDistance * maxDistance;
        for (int y = Math.Max(0, cy - cellRadius); y <= Math.Min(GridHeight - 1, cy + cellRadius); y++)
        {
            for (int x = Math.Max(0, cx - cellRadius); x <= Math.Min(GridWidth - 1, cx + cellRadius); x++)
            {
                if (!RouteMask[y * GridWidth + x])
                {
                    continue;
                }

                double dx = tileX - (x * CellSize + CellSize / 2d);
                double dy = tileY - (y * CellSize + CellSize / 2d);
...
internal readonly record struct RouteBridge(int X1, int Y1, int X2, int Y2);
...
internal static class WallIds
{
    public const ushort JungleHouse = 42;
    public const ushort JungleHouseAged = 64;
    public const ushort LivingMahogany = 78;
    public const ushort Hive = 86;
    public const ushort LihzahrdBrick = 87;
    public const ushort HiveSafe = 108;
}

internal static class ItemIds
{
    public const int LifeCrystal = 29;
    public const int CloudinaBottle = 53;
    public const int HermesBoots = 54;
    public const int Sapphire = 177;
    public const int Ruby = 178;
    public const int Emerald = 179;
    public const int Topaz = 180;
    public const int Amethyst = 181;
    public const int Diamond = 182;
    public const int FeralClaws = 211;
    public const int AnkletOfTheWind = 212;
    public const int FeatherfallPotion = 295;
    public const int SpelunkerPotion = 296;
    public const int SandstormInABottle = 857;
    public const int FlyingCarpet = 934;
    public const int Boomstick = 964;
    public const int LivingLoom = 2196;
    public const int BeeMinecart = 4426;
}
