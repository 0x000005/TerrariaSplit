...
internal sealed class RaceProgressTransport : IDisposable
{
    ...
    public RaceProgressTransport(
        RaceClientSession session,
        ...
        bool runStarted)
    {
        ...
        DateTimeOffset reportedAtUtc = timeProvider.GetUtcNow();
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested && IsCurrent(upload))
        {
            ...
            {
                logInfo("Race progress upload will retry: " + ex.Message);
            }
            catch (Exception ex)
            {
                logError(ex, "Race progress upload failed and will retry.");
            }
        ...
        string operation)
    {
        ...
        logInfo($"Race {operation} rejected. Error={errorCode} Message={message}.");
    ...
    private abstract record ProgressUpload
    {
        ...
        public static ProgressUpload ForReset(RaceProgressResetRequest request) => new Reset(request);

        public static ProgressUpload ForStart(RaceRunStartReport report) => new Start(report);

        public static ProgressUpload ForSplit(RaceSplitReport report) => new Split(report);

        public static ProgressUpload ForDeath(RaceDeathReport report) => new Death(report);
    }
