...
`WorldGenSim` is the historical isolated workspace for the first-stage Terraria
worldgen replica.

The authoritative fast pre-screen implementation has since been integrated into
the main TerrariaSplit project under:

```text
TerrariaSplit/Terraria/WorldGeneration/Simulation
```

Use that namespace, its tests, and
`TerrariaSplit/Terraria/WorldGeneration/Simulation/PyramidDependencyAudit.md`
for current behavior. This folder is not included by `TerrariaSplit.slnx` and
may lag behind the integrated simulator.
...
The current integrated correctness target is small-world, crimson, normal
non-secret seeds. The pyramid search area is `X=[32%,68%)` and `Y=[15%,35%)` of
the world. A seed is only considered matched when the simulator and the
reference world agree on:
...
The integrated replica backend has the official normal-world pass order wired
through `Pyramids`. Most pass bodies needed before `Pyramids` are implemented,
locally simulated, or paired with conservative candidate risk gates for the
target scope. The normal small-world crimson branch of `Corruption` is modeled
because crimson can mutate sand inside the target area, and that can change
whether the official `Pyramids` pass accepts a candidate.
...
For stage 1, pass-level skips are limited to audited sky-only, cavern-only,
bottom-only, far-edge, wall-only, sand-protected, or post-target generation
whose remaining uncertainty is either outside the target pyramid area or covered
by a conservative candidate risk gate.
...
The current maintained baseline is in the integrated simulator, not this
standalone folder. See
`TerrariaSplit/Terraria/WorldGeneration/Simulation/PyramidDependencyAudit.md`
for the current known probe seeds and risk gates.
